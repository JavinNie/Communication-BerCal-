...
num_pattern = 50;
for i = 1:length(Array)/floor(sample_per_baud)/250/num_pattern
...
for q = 1:length(Array)/floor(sample_per_baud)/250/num_pattern
...
%     Bstream=Array(round([ind:sample_per_baud:end]));%按最佳采样点采样
    ...
    Bstream_source=repmat(Bstream_source,num_pattern,1);
